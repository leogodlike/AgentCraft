...
class CronStore:
    ...
    def create_job(self, job: CronJob) -> CronJob:
        ...
        try:
            ...
            """, (
                job.id, job.name, 1 if job.enabled else 0,
                json.dumps(schedule_to_dict(job.schedule)), job.task, job.agent_type,
                job.session_target.value, json.dumps(self._delivery_to_dict(job.delivery)),
                job.timeout, job.max_retries, job.created_at, job.updated_at,
                job.state.status.value, job.state.run_count, job.state.error_count,
            ))
            conn.commit()
    ...
    def update_job(self, job: CronJob) -> CronJob:
        ...
        try:
            conn.execute("""
                ...
                WHERE id=?
            """, (
                job.name, 1 if job.enabled else 0,
                json.dumps(schedule_to_dict(job.schedule)), job.task, job.agent_type,
                job.session_target.value, json.dumps(self._delivery_to_dict(job.delivery)),
                job.timeout, job.max_retries, job.updated_at,
                job.state.status.value, job.state.last_run_at, job.state.next_run_at,
                job.state.last_result, job.state.last_error, job.state.run_count, job.state.error_count, job.id,
            ))
            conn.commit()
    ...
    def update_state(self, job_id: str, state: CronJobState) -> None:
        ...
        try:
            conn.execute("""
                ...
                WHERE id=?
            """, (
                state.status.value, state.last_run_at, state.next_run_at,
                state.last_result, state.last_error, state.run_count, state.error_count, job_id,
            ))
            conn.commit()
        finally:
            conn.close()
    ...
    def record_run(self, telemetry: dict[str, Any]) -> None:
        ...
        try:
            ...
            """, (
                run_id, telemetry["job_id"], telemetry["run_at"],
                telemetry.get("duration_ms"), telemetry["status"],
                telemetry.get("model"), telemetry.get("provider"),
                telemetry.get("input_tokens"), telemetry.get("output_tokens"),
                telemetry.get("total_tokens"), telemetry.get("error"), telemetry.get("result"),
            ))
            conn.commit()
        finally:
            conn.close()
    ...
    def _row_to_job(self, row: sqlite3.Row) -> CronJob:
        ...
        return CronJob(
            ...
            schedule=schedule, task=row["task"], agent_type=row["agent_type"],
            session_target=SessionTarget(row["session_target"]), delivery=delivery,
            state=state, timeout=row["timeout"], max_retries=row["max_retries"],
    ...
    def _delivery_to_dict(self, delivery: CronDelivery) -> dict[str, Any]:
        ...
        return {
            "mode": delivery.mode.value, "channel": delivery.channel, "to": delivery.to,
            "webhook_url": delivery.webhook_url, "thread_id": delivery.thread_id,
            "account_id": delivery.account_id, "best_effort": delivery.best_effort,
            "failure_channel": delivery.failure_channel, "failure_to": delivery.failure_to,
        }
    ...
    def _dict_to_delivery(self, d: dict[str, Any]) -> CronDelivery:
        ...
        return CronDelivery(
            ...
            channel=d.get("channel"), to=d.get("to"), webhook_url=d.get("webhook_url"),
            thread_id=d.get("thread_id"), account_id=d.get("account_id"),
            best_effort=d.get("best_effort", False),
            failure_channel=d.get("failure_channel"), failure_to=d.get("failure_to"),
        )